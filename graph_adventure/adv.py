...
def dft(player):
    ...
    while s.size() > 0:
        ...
        if "?" not in list(traversal_graph[player.currentRoom.id].values()):
            # BFS should take a current room and target room is the last room in stack
            # and it will return the list of directions from current room to target room
            try:
                target = player.currentRoom.getRoomInDirection(s.stack[-1])
                if player.currentRoom.id == target.id:
                    target = player.currentRoom.getRoomInDirection(s.stack[-2])
                    completed = bfs(player.currentRoom, target)
                else:
                    completed = bfs(player.currentRoom, target)
                for direction in range(1, len(completed)-1):
                    traversalPath.append(completed[direction])
            except IndexError:
                print("We are done - we think")
...
def bfs(starting_vertex, destination_vertex):
    ...
    while q.size() > 0:
        v = q.dequeue()
        if v[0].id not in visited:
            visited.add(v[0].id)
            if v[0].id == destination_vertex.id:
                # append direction to destination index
                # use get exits and check if the room is target => we have direction
                for direction in v[0].getExits():
                    if v[0].getRoomInDirection(direction) == destination_vertex.id:
                        v[1].append(direction)
                        print("bfs_list:", v[1])
                        return v[1]

        for direction in starting_vertex.getExits():
            room = v[0].getRoomInDirection(direction)
            path_to_neigbor = v[1].copy()
            path_to_neigbor.append(direction)
            q.enqueue((room, path_to_neigbor))
...
#         # print(traversal_graph[player.currentRoom.id].values())
#         if "?" not in list(traversal_graph[player.currentRoom.id].values()):
#             noUnexplored = True
